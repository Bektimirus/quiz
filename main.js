...
const numberOfQuestion = document.getElementById('number-of-question'), //номер вопроса
      numberOfAllQuestions = document.getElementById('number-of-all-questions'); // количестов всех вопросов
...
const questions = [
      {
            question: 'Сколько раз ID элемента может повторяться в коде документа?',
            options: [
                  'Сколько угодно',
                  "Только один раз",
                  'Два раза',
      ...
      {
            question: 'Как получить число Пи?',
            options: [
                  'Number.pi',
                  'pi',
                  'Math.pi',
      ...
      {
            question: 'Какая функция заменяет собой много вызовов условий else - if?',
            options: [
                  'switch...case',
                  "try...catch",
                  'Тернарные операторы',
...
numberOfQuestion.innerHTML = indexOfPage +1; //установка номера тек-й страницы
...
const validate = () => {
      if(!optionElements[0].classList.contains('disabled')){
            alert('Выберите идин из вариантов ответа');
      } else {
            randomQuestion();
            enableOptions();
      }
